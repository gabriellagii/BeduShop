...
const SchemaUsuario = mongoose.Schema({
    ...
    username:{
        ...
        required: true,
        lowercase: true,
        match: [/^[a-zA-Z0-9]+$/, "Usuario inválido"],
        index: true
    },
    hash: String,
    salt: String,
...
SchemaUsuario.methods.crearContrasena = function(password){//crear contraseñas
...
SchemaUsuario.methods.validarContrasena = function(password){//valida el texto , compara 
    const pass = crypto
    .pbkdf2Sync(password, this.salt, 10000, 512, "sha512")
    .toString("hex")
